module CollegeImporter
  ...
  module FSU
    class Import
      attr_reader :classes, :cookie, :departments
      def initialize(term = "20126")
        if term[-1] == "6"
          @table_headers = %w{ course_number section course_ref_num summer title instructor seats seats_left building room days begin end  }
        else
      ...
      def parse_request_string(department)
        tmp = "requestType=PUBLIC&courseNumber=&term=#{@term}&department=#{department}&level=&location=0100FSU+Main+Campus&specialProgram=-9999&beginningTime=&endingTime=&searchCriteria=-9999&criteriaDesc="
        tmp_hash = {}
        tmp.split('&').each do |s|
          tmp_x = s.split('=')
      ...
      def fetch_department(department)
        ...
        begin
          ...
          request.set_form_data(parse_request_string(department))
          HDRS.each do |k, v|
      ...
      def parse_dept_html(response)
          begin
            ...
            url = ""
            is_class_row = false
            for x in @web_doc.search('table tr td table tr')
              content = ""
              xhash = {}
              ...
              x.search('td').each_with_index do |xx, i|
                ...
                if is_class_row 
                  # skip rows that are Comment rows
                  next if xx.inner_html.include?('Comment')
                end
                ...
                # special logic for summer view in bb
                if (i >= 12 && @term[-1] != "6") || (i >= 13 && @term[-1] == "6")
                  is_class_row = false
                end
              end

              local_classes << xhash unless xhash.size == 0

            end

            local_classes = local_classes.drop(1)
      ...
      def parse_departments
        ...
        @departments.each do |d|
          ...
          begin
            response = fetch_department(d)
            parse_dept_html(response)
          rescue Exception => e
            p "Exception parsing #{d}: #{e}"
          end
          i += 1
          sleep 1
